...

{
    ...
    std::string input;
    cout << "Enter Name: ";
    regex regjunir("^[A-Z]([a-z,',-]*)");
    //  READ LINE TO ENTER EMPLOYEE'S NAME
    getline(cin, emp.Name);
    while (!(regex_match(emp.Name, regjunir)))
    {
        cout << "Incorrect format re_enter Employee Name: ";
        getline(cin, emp.Name);
    ...
    regex regdob("\\b\\d{4}/\\d{2}/\\d{2}\\b");
    std::string input;
    cout << "Enter DOB: ";
    //Read employee dob from keyboard
    getline(cin, emp.DOB);
    while (!(regex_match(emp.DOB, regdob)))
    {
        cout << "Incorrect format re_enter employee dob: ";
        getline(cin, emp.DOB);
    }

    // READ LINE TO ENTER EMPLOYEE'S TELEPHONE

    regex regnum("^0([0-9]{10})");
    //std::string input;
    cout << "Enter tel: ";
    getline(cin, emp.Tel);
    while (regex_match(emp.Tel, regnum))
    {
        cout << "Incorrect format re_enter employee tel: ";
        getline(cin, emp.Tel);
    }
   ...
   // READ LINE TO ENTER EMPLOYEE'S POSTCODE
    regex regPostcode("^([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([AZa-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9]?[A-Za-z]))))[0 - 9][A - Za - z]{ 2 })$");
    std::string input;
    cout << "Enter Postcode: ";
    getline(cin, emp.Postcode);
    while (regex_match(emp.Postcode, regPostcode))
    {
        cout << "Incorrect format re_enter employee Postcode: ";
        getline(cin, emp.Postcode);
...

{
...

    {
...

        {
            ...
            cout << "The checksum number is : " << generate_checksum(emp) << endl;
